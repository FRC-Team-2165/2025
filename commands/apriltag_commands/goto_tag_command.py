...
class GotoTagCommand(Command):
    def __init__(self, subsystem: DriveSubsystem, location_stream: LocationDataClientManager, target_id: int, loss_timeout: float = 0.2, drive_speed: float = 0, x_offset: float = 0, y_offset: float = 0, deadband: float = 0.1):
        super().__init__()
    ...
    def execute(self):
        ...
        if self.has_target or abs(process_time() - self.last_seen) < self.loss_timeout:
            ...
            x_speed = 0
            if abs(current.x) < self.deadband:
                pass
            else:
            ...
            y_speed = 0
            if abs(current.y) < self.deadband:
                pass
            else:
            ...
            self.x_speed = 0
            self.y_speed = 0
        self.subsystem.drive(self.x_speed, self.y_speed, 0)
